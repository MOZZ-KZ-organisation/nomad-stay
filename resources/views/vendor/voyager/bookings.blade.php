...
@section('content')
<link rel="stylesheet" href="{{ mix('css/vendor/voyager/booking.css') }}">

<div class="top-controls">
    <h1>
        Календарь бронирований
        <span>{{ $dates->first()->format('d.m') }} – {{ $dates->last()->format('d.m') }}</span>
    </h1>
    ...
    @foreach($rooms as $room)
        <div class="calendar-row">
            ...
            <div class="row-body">
                ...
                {{-- Hover промежутки между ячейками --}}
                @for($i = 0; $i < 18-1; $i++)
...
.calendar-wrapper {
    background: #fff; border-radius: 12px; margin: 16px; overflow: hidden;
}
.calendar-header, .calendar-row { display: flex; position: relative; border: 1px solid #e5e7eb; border-bottom: none; }
.room-col { width: 160px; padding: 10px; border-right: 2px solid #e5e7eb; background: #fafafa; flex-shrink: 0; }
.day-col { flex: 1; text-align: center; padding: 6px 0; font-size: 12px; border-right: 1px solid #e5e7eb; }
.day-week { font-size: 11px; color: #6b7280; }
.calendar-row { height: 38px; border-top: 1px solid #e5e7eb; border-bottom: 1px solid #e5e7eb; border-left: 1px solid #e5e7eb; }
.row-body { position: relative; flex: 1; height: 38px; }

.day-bg { position: absolute; top: 0; bottom: 0; background: #eff6ff; border-right: 1px solid #e5e7eb; }
.booking-bar { position: absolute; top: 0; bottom: 0; border-radius: 1px; padding: 4px 8px; font-size: 12px; font-weight: 500; color: #111; display: flex; align-items: center; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; cursor: pointer; z-index: 10; transition: filter 0.2s ease; }
.booking-bar:hover { filter: brightness(95%); }

/* Hover промежутки */
.hover-slot { position: absolute; top:0; bottom:0; background:transparent; z-index:1; cursor:pointer; transition: background 0.2s; }
.hover-slot:hover { background: rgba(0,0,0,0.05); }

/* Остальные стили для фильтров и уведомлений */
.filters-wrapper { position: relative; }
.filter-panel { display: none; position: absolute; top: 45px; width: 280px; background: #fff; border-radius: 12px; box-shadow: 0 8px 25px rgba(0,0,0,0.08); padding: 15px; z-index: 999; }
.room-hotel { font-size: 11px; color: #6b7280; }
.notifications-wrapper { position: relative; }
.icon-btn { width: 40px; height: 40px; border-radius: 50%; border: 1px solid #e5e7eb; background: #fff; cursor: pointer; position: relative; font-size: 16px; }
#notificationCount { position: absolute; top: -6px; right: -6px; background: #ef4444; color:#fff; border-radius:999px; font-size:11px; padding:2px 6px; display:none; line-height:1; }
.dropdown-panel { display:none; position:absolute; top:48px; right:0; width:320px; background:#fff; border-radius:12px; box-shadow:0 10px 30px rgba(0,0,0,.12); padding:12px; z-index:9999; }
...
.nav-arrow { display:flex; justify-content:center; align-items:center; width:36px; height:36px; border-radius:8px; background:#f3f4f6; border:1px solid #d1d5db; text-decoration:none; font-weight:bold; font-size:18px; color:#111; cursor:pointer; transition:all 0.2s ease; }
